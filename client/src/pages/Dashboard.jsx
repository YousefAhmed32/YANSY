...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (!window.location.pathname.startsWith('/__visual/')) dispatch(fetchProjects());
  }, [dispatch]);
  ...
  return <div className="client-dashboard" dir={isRTL ? 'rtl' : 'ltr'} style={{ fontFamily: FONT(isRTL), color: TK.text }}>
    ...
    <div className="cd-main">
      <section className="cd-card cd-project"><div className="cd-project-copy"><div style={{ display: 'flex', justifyContent: 'space-between', gap: 16 }}><span style={{ fontSize: 11, letterSpacing: ar ? 0 : '.12em', color: 'rgba(255,255,255,.62)', fontWeight: 650 }}>{T.current}</span>{activeProject && <Badge tone={status[0]} dot>{ar ? status[1] : status[2]}</Badge>}</div><h2 style={{ fontSize: 'clamp(28px,4vw,44px)', lineHeight: 1.2, maxWidth: 580, margin: '34px 0 10px', letterSpacing: ar ? '-.035em' : '-.045em' }}>{activeProject?.name || activeProject?.title || (ar ? 'مشروعك الحالي' : 'Your current project')}</h2><p style={{ color: 'rgba(255,255,255,.66)', fontSize: 13 }}>{T.progress}</p><div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 14 }}><div style={{ height: 7, flex: 1, maxWidth: 410, borderRadius: 10, background: 'rgba(255,255,255,.12)', overflow: 'hidden' }}><div style={{ width: `${progress}%`, height: '100%', background: 'linear-gradient(90deg,#3B82F6,#fff)' }}/></div><strong dir="ltr">{progress}%</strong></div><div style={{ marginTop: 34 }}><Link className="cd-btn" to={activeProject?._id ? `/app/projects/${activeProject._id}` : '/app/projects'} style={{ background: '#fff', color: '#172554' }}>{T.open}<Arrow size={15}/></Link></div></div><div className="cd-visual" aria-hidden="true"><img src="/assets/image/yansy-command-center-3d-v1.png" alt="" /></div></section>
