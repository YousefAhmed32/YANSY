  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-white/5">
        ...
        {/* Arrow badge */}
        <div className="absolute top-4 right-4 w-9 h-9 border border-white/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 -translate-y-2 group-hover:translate-y-0 bg-black/40 backdrop-blur-sm">
          <ArrowUpRight className="w-4 h-4 text-white" />
        </div>

        {/* Category badge */}
        <div className="absolute top-4 left-4">
          <span className="text-[10px] px-3 py-1 border border-[#d4af37]/40 text-[#d4af37] tracking-widest uppercase bg-black/40 backdrop-blur-sm">
            {p.category}
          </span>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5">
          <h3
            className="text-white font-semibold text-lg sm:text-xl leading-tight mb-1 transition-transform duration-300 group-hover:-translate-y-1"
            style={{ letterSpacing: '-0.015em' }}
          >
            {isRTL && p.titleAr ? p.titleAr : p.title}
          </h3>
          <p className="text-white/50 text-xs font-light line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      ...
      {p.tags?.length > 0 && (
        <div className={`flex flex-wrap gap-1.5 mt-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
          {p.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="text-[10px] px-2 py-0.5 border border-white/10 text-white/30 tracking-wide"
            >
              {tag}
            </span>
          ))}
    ...
    <div className="bg-black text-white min-h-screen overflow-x-hidden" dir={dir}>
      ...
      <section ref={heroRef} className="relative pt-40 pb-24 px-4 sm:px-8 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <p
            data-hero
            className="text-xs tracking-[0.4em] text-[#d4af37]/60 uppercase mb-6 opacity-0"
          >
          ...
          <p data-hero className="opacity-0 text-white/60 font-normal text-base sm:text-xl max-w-2xl">
            {isRTL
      ...
      {/* ── FILTER ────────────────────────────────────────────────────────── */}
      <div className="sticky top-[72px] z-30 bg-black/90 backdrop-blur-md border-b border-white/5 px-4 sm:px-8 py-4">
          ...
          <div className={`flex gap-2 min-w-max ${isRTL ? 'flex-row-reverse' : ''}`}>
            {CATEGORIES.map((cat) => (
              <button
                ...
                style={{
                  borderColor: active === cat.en ? '#d4af37' : 'rgba(255,255,255,0.1)',
                  color: active === cat.en ? '#d4af37' : 'rgba(255,255,255,0.4)',
                  background: active === cat.en ? 'rgba(212,175,55,0.08)' : 'transparent',
                }}
              >
                {activeLabel(cat)}
              </button>
      ...
      <section className="px-4 sm:px-8 py-16 sm:py-24">
        <div className="max-w-7xl mx-auto">
          {loading ? (
            ...
            <div className="text-center py-32 text-white/20 font-light text-xl">
      ...
      <section className="relative px-4 sm:px-8 py-32 sm:py-40 overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center relative z-10">
          ...
          {/* headline */}
          <h2
            className="font-bold tracking-tight leading-[1.08] mb-6 text-white"
            style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', letterSpacing: '-0.03em' }}
          >
            {isRTL ? (
              <>مشروعك القادم<br /><span className="text-[#d4af37]">يستحق الأفضل</span></>
            ) : (
              <>Your next project<br /><span className="text-[#d4af37]">deserves the best.</span></>
            )}
          </h2>

          {/* sub */}
          <p className="text-white/30 font-light text-base sm:text-lg mb-12 max-w-xl mx-auto leading-relaxed">
            {isRTL
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              ...
              className="group inline-flex items-center gap-3 px-10 py-5 bg-[#d4af37] text-black text-xs font-light tracking-widest uppercase hover:bg-white transition-all duration-500"
            >
            ...
            <a
              ...
              className="group inline-flex items-center gap-3 px-10 py-5 border border-[#d4af37] text-[#d4af37] text-xs font-light tracking-widest uppercase hover:bg-[#d4af37] hover:text-black transition-all duration-500"
            >
          ...
          <p className="mt-6 text-white/15 text-[11px] tracking-widest">
