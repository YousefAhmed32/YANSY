  ...
  return (
    <div style={{ padding: '22px', background: TK.surface, border: `1px solid ${TK.border}`, borderRadius: '12px' }}>
      ...
      {items.map((b, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '10px 0', borderBottom: i < items.length - 1 ? `1px solid rgba(0,0,0,0.04)` : 'none' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '12px', fontWeight: 300, color: TK.text }}>{b.title}</div>
            ...
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              ...
              <div style={{ fontSize: '10px', color: TK.textMuted }}>{b.target || 'all'}</div>
            </div>
          </div>
        );
      })}
...
const AdminNotifications = () => {
  ...
  const inputStyle = {
    width: '100%', padding: '10px 12px', background: TK.inputBg,
    border: `1px solid ${TK.border}`, borderRadius: '8px',
    color: TK.text, fontSize: '12px', outline: 'none',
    ...
    <div style={{ minHeight: '100vh', background: TK.bg, color: TK.text, padding: '32px 32px 60px' }}>
      ...
      <div style={{ marginBottom: '32px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '3px 10px', borderRadius: '20px', border: '1px solid rgba(37,99,235,0.25)', background: 'rgba(37,99,235,0.06)', marginBottom: '10px' }}>
          <Bell style={{ width: '10px', height: '10px', color: TK.accent }} />
          <span style={{ fontSize: '10px', fontWeight: 400, color: TK.accent, letterSpacing: '0.12em', textTransform: 'uppercase' }}>Communication</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '24px' }}>
        {/* Compose panel */}
        <div>
          <div style={{ padding: '24px', background: TK.surface, border: `1px solid ${TK.border}`, borderRadius: '12px', marginBottom: '20px' }}>
            <h2 style={{ fontSize: '12px', fontWeight: 400, color: TK.text, letterSpacing: '0.08em', textTransform: 'uppercase', margin: '0 0 20px' }}>
              Compose Broadcast
            </h2>

            {/* Type */}
            <div style={{ marginBottom: '18px' }}>
              <label style={{ display: 'block', fontSize: '11px', color: TK.textMuted, marginBottom: '8px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Type</label>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {TYPES.map(t => (
                  <button
                    key={t.value}
                    onClick={() => setForm(f => ({ ...f, type: t.value }))}
                    style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 12px', borderRadius: '8px', background: form.type === t.value ? `${t.color}15` : 'transparent', border: `1px solid ${form.type === t.value ? t.color + '40' : TK.border}`, color: form.type === t.value ? t.color : TK.textMuted, fontSize: '11px', cursor: 'pointer', transition: 'all 0.15s' }}
                  >
                    <t.icon size={12} />
                    {t.label}
                  </button>
            ...
            {/* Target */}
            <div style={{ marginBottom: '18px' }}>
              <label style={{ display: 'block', fontSize: '11px', color: TK.textMuted, marginBottom: '8px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Audience</label>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {TARGETS.map(t => (
                  <button
                    key={t.value}
                    onClick={() => setForm(f => ({ ...f, targetRole: t.value }))}
                    style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 12px', borderRadius: '8px', background: form.targetRole === t.value ? 'rgba(37,99,235,0.1)' : 'transparent', border: `1px solid ${form.targetRole === t.value ? 'rgba(37,99,235,0.4)' : TK.border}`, color: form.targetRole === t.value ? TK.accent : TK.textMuted, fontSize: '11px', cursor: 'pointer', transition: 'all 0.15s' }}
                  >
                    <t.icon size={12} />
                    {t.label}
                  </button>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label style={{ display: 'block', fontSize: '11px', color: TK.textMuted, marginBottom: '6px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Title *</label>
              <input
                value={form.title}
            ...
            {/* Message */}
            <div style={{ marginBottom: '14px' }}>
              <label style={{ display: 'block', fontSize: '11px', color: TK.textMuted, marginBottom: '6px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Message *</label>
              <textarea
                value={form.message}
            ...
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '11px', color: TK.textMuted, marginBottom: '6px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Action Link (optional)</label>
              <input
                value={form.link}
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button
                ...
                style={{ padding: '10px 18px', background: 'transparent', border: `1px solid ${TK.border}`, borderRadius: '8px', color: TK.textMuted, fontSize: '11px', cursor: 'pointer', transition: 'all 0.2s' }}
              >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {preview && (
            <div style={{ padding: '22px', background: TK.surface, border: `1px solid ${TK.border}`, borderRadius: '12px' }}>
              <h3 style={{ fontSize: '10px', fontWeight: 400, color: TK.textMuted, letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 14px' }}>Preview</h3>
              <div style={{ padding: '14px', background: `${selectedType.color}10`, border: `1px solid ${selectedType.color}25`, borderRadius: '10px' }}>
                ...
                <p style={{ fontSize: '11px', color: TK.textMuted, margin: 0, fontWeight: 300 }}>{form.message || 'Your message will appear here...'}</p>
                {form.link && <div style={{ fontSize: '10px', color: TK.accent, marginTop: '8px' }}>→ {form.link}</div>}
              </div>
              <div style={{ marginTop: '10px', fontSize: '10px', color: TK.textMuted }}>
                To: <strong style={{ color: TK.text }}>{selectedTarget.label}</strong> · {selectedTarget.description}
              </div>
            </div>
          )}

          {/* Tips */}
          <div style={{ padding: '18px 20px', background: TK.surface, border: `1px solid ${TK.border}`, borderRadius: '12px' }}>
            <h3 style={{ fontSize: '10px', fontWeight: 400, color: TK.textMuted, letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 14px' }}>Tips</h3>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: '10px', marginBottom: '10px', alignItems: 'flex-start' }}>
                ...
                <span style={{ fontSize: '11px', color: TK.textMuted, fontWeight: 300 }}>{text}</span>
              </div>
            ))}
          </div>
          ...
          {/* Severity guide */}
          <div style={{ padding: '18px 20px', background: TK.surface, border: `1px solid ${TK.border}`, borderRadius: '12px' }}>
            <h3 style={{ fontSize: '10px', fontWeight: 400, color: TK.textMuted, letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 14px' }}>Severity Guide</h3>
            {TYPES.map(t => (
              <div key={t.value} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: '11px', color: TK.textMuted, marginLeft: '5px', fontWeight: 300 }}>
