...
import { Shield, RefreshCw, Filter, ChevronRight } from 'lucide-react';
    ...
    <div dir={dir} style={{ minHeight: '100vh', background: bg, color: textMain, padding: '32px 32px 60px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <h1 style={{ fontSize: 'clamp(22px,3vw,34px)', fontWeight: 600, color: textMain, margin: 0, fontFamily: "'Inter',system-ui,sans-serif" }}>
            Audit Log
          </h1>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '20px' }}>
        <select
          ...
          style={{
            ...
            borderRadius: '8px', color: textMuted, fontSize: '12px', outline: 'none',
          }}
        >
        ...
        <input
          ...
          style={{ padding: '8px 12px', background: surface, border: `1px solid ${border}`, borderRadius: '8px', color: textMuted, fontSize: '12px', outline: 'none' }}
        />
        <input
          type="date"
        ...
        <button
          ...
          style={{
            ...
            padding: '8px 14px', background: 'rgba(212,175,55,0.1)',
      ...
      {loading ? (
        ...
        <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: '12px', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            fontSize: '9px', fontWeight: 400, color: textMuted,
          ...
          {logs.length === 0 ? (
            ...
            logs.map((log, i) => (
              <div key={log._id} style={{
                ...
                borderBottom: i < logs.length - 1 ? `1px solid ${isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)'}` : 'none',
                transition: 'background 0.1s',
              }}
                onMouseEnter={e => { e.currentTarget.style.background = isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
              >
                <span style={{ fontSize: '10px', color: textMuted }}>{timeFormat(log.createdAt)}</span>
                <span style={{
                ...
                <div>
                  <div style={{ fontSize: '11px', color: textMain }}>{log.actor?.fullName || log.actorEmail || 'Unknown'}</div>
                  <div style={{ fontSize: '9px', color: textMuted, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{log.actorRole}</div>
                </div>
                <span style={{ fontSize: '10px', color: textMuted }}>{log.entityType}</span>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '20px' }}>
          <button
            ...
            disabled={page <= 1}
            style={{ padding: '6px 14px', background: surface, border: `1px solid ${border}`, borderRadius: '6px', color: textMuted, fontSize: '11px', cursor: page <= 1 ? 'not-allowed' : 'pointer', opacity: page <= 1 ? 0.4 : 1 }}
          >
            Previous
          </button>
          <span style={{ padding: '6px 10px', fontSize: '11px', color: textMuted }}>Page {page} of {totalPages}</span>
          <button
            onClick={() => fetchLogs(page + 1)}
            disabled={page >= totalPages}
            style={{ padding: '6px 14px', background: surface, border: `1px solid ${border}`, borderRadius: '6px', color: textMuted, fontSize: '11px', cursor: page >= totalPages ? 'not-allowed' : 'pointer', opacity: page >= totalPages ? 0.4 : 1 }}
          >
            Next
          </button>
        </div>
