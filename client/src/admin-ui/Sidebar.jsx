...
const buildAdminNav = (t, language) => {
  ...
  return [
    ...
    { to: '/app/admin/media-library',       label: ar ? 'مكتبة الوسائط'      : 'Media Library',       icon: Library,     group: g.libraries },
    { to: '/app/admin/libraries/team',       label: ar ? 'أعضاء الفريق'      : 'Team Members',        icon: UserSquare2, group: g.libraries },
    { to: '/app/admin/libraries/clients',    label: ar ? 'العملاء'           : 'Clients',             icon: Building2,   group: g.libraries },
    { to: '/app/admin/libraries/technologies', label: ar ? 'التقنيات'        : 'Technologies',        icon: Cpu,         group: g.libraries },
    { to: '/app/admin/libraries/tags',       label: ar ? 'وسوم المشاريع'     : 'Project Tags',        icon: TagIcon,     group: g.libraries },
    { to: '/app/admin/libraries/testimonials', label: ar ? 'الشهادات'        : 'Testimonials',        icon: Quote,       group: g.libraries },
    { to: '/app/admin/libraries/awards',     label: ar ? 'الجوائز والشهادات' : 'Awards',              icon: Award,       group: g.libraries },
...
const RECENT_CAP = 4;
// One consistent neutral tone for every nav icon — the old per-item rainbow
// (six hues cycled by path-hash) read as decorative noise rather than
// meaning; the approved reference uses plain outline icons throughout.
...
const Sidebar = ({
    ...
    return (
      <Link
        ...
        style={{
          ...
          color: active ? TK.inkFg : (hov ? TK.text : TK.textMuted),
          background: active ? TK.ink : (hov ? TK.hoverBg : 'transparent'),
          borderRadius: '10px',
          transition: `background ${MOTION.fast} ${MOTION.ease}, color ${MOTION.fast} ${MOTION.ease}`,
          position: 'relative', opacity: dim ? 0.55 : 1,
        }}
      >
        <span style={{ width: 28, height: 28, borderRadius: 8, display: 'grid', placeItems: 'center', flexShrink: 0, color: active ? TK.inkFg : TK.textMuted }}>
          <Icon style={{ width: '16px', height: '16px', strokeWidth: 1.8 }} />
        </span>
        {!collapsed && (
          <span style={{
  ...
  const renderBody = (isMobileDrawer) => {
    ...
    return (
      <div className="yansy-sidebar-body" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        {/* ── Workspace header ── */}
        <div style={{ padding: isCollapsed ? '13px 0' : '16px 12px 13px', borderBottom: `1px solid ${TK.border}`, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: isCollapsed ? 'center' : 'flex-start' }}>
            ...
            >
              <div style={{
                width: isCollapsed ? '38px' : '40px', height: '36px', borderRadius: '10px', flexShrink: 0,
                background: TK.bgSubtle, border: `1px solid ${TK.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center',
                overflow: 'hidden',
              }}>
                <img src="/assets/image/logo/favicon-96x96.png" alt="YANSY" style={{ width: '28px', height: '28px', objectFit: 'contain' }} />
              </div>
              {!isCollapsed && (
                <div style={{ minWidth: 0, flex: 1 }}>
                  {/* Brand name is always Latin text — without an explicit ltr
                      isolation it inherits the shell's `direction: rtl` in
                      Arabic mode, so an overflow ellipsis clips from the front
                      ("…SY Tech") instead of the end. */}
                  <div dir="ltr" style={{ fontSize: '14px', fontWeight: 800, color: TK.text, letterSpacing: '-0.025em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textAlign: isRTL ? 'right' : 'left' }}>
                    YANSY Tech
                  </div>
                  <div style={{ fontSize: '10px', color: TK.textLight, whiteSpace: 'nowrap' }}>
                    {isAdmin ? (isRTL ? 'مساحة الإدارة' : 'Admin Workspace') : (isRTL ? 'مساحة العمل' : 'Workspace')}
                  </div>
                </div>
              )}
            ...
            {/* "Float sidebar" is an admin power-user affordance (dense nav,
                heavy resizing); the client workspace has 8 static links and
                doesn't need it — hiding it for clients also recovers width
                the brand name needs to avoid truncating. */}
            {isAdmin && !isMobileDrawer && !isCollapsed && (
              <button
          ...
          {/* ── In-sidebar search ── */}
          {!isCollapsed && (
            <div className="au-input yansy-sidebar-search" style={{ marginTop: '12px', display: 'flex', alignItems: 'center', gap: '8px', background: TK.bgSubtle, border: `1px solid ${TK.border}`, borderRadius: '10px', padding: '9px 11px' }}>
              <Search style={{ width: 14, height: 14, color: TK.textLight, flexShrink: 0 }} />
              <input
                value={filter}
        ...
        {/* ── Footer ── */}
        <div style={{ borderTop: `1px solid ${TK.border}`, padding: '7px 5px 5px', flexShrink: 0 }}>
          <button
            onClick={onOpenSearch}
            aria-label={isRTL ? 'بحث شامل' : 'Global search'}
            className="au-icon-btn"
            style={{ display: 'flex', alignItems: 'center', gap: '9px', width: '100%', padding: isCollapsed ? '9px 0' : '9px 12px', justifyContent: isCollapsed ? 'center' : 'flex-start', background: 'transparent', border: 'none', cursor: 'pointer', color: TK.textMuted }}
          >
            <Search style={{ width: 13.5, height: 13.5, opacity: 0.7, flexShrink: 0 }} />
            {!isCollapsed && (
              <>
                <span style={{ flex: 1, fontSize: '12px', textAlign: isRTL ? 'right' : 'left' }}>{isRTL ? 'بحث شامل' : 'Search everywhere'}</span>
                <kbd style={{ padding: '2px 6px', borderRadius: '5px', fontSize: '9px', background: TK.bgSubtle, border: `1px solid ${TK.border}`, color: TK.textLight, fontFamily: 'monospace' }}>⌘K</kbd>
              </>
            )}
          </button>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '9px', width: '100%', padding: isCollapsed ? '9px 0' : '9px 12px', justifyContent: isCollapsed ? 'center' : 'flex-start', background: 'transparent', border: 'none', cursor: 'pointer', color: TK.textMuted }}
          >
            <Globe style={{ width: 13.5, height: 13.5, opacity: 0.7, flexShrink: 0 }} />
          ...
          <div ref={userMenuRef} style={{ position: 'relative' }}>
            <button
              ...
              style={{
                ...
                border: isCollapsed ? 'none' : `1px solid ${TK.border}`,
                borderRadius: isCollapsed ? 0 : '12px', cursor: 'pointer', color: TK.textMuted,
                transition: `all ${MOTION.fast} ${MOTION.ease}`,
              }}
              className="au-icon-btn"
            >
              <Avatar name={user?.fullName} email={user?.email} size={30} />
              {!isCollapsed && (
                <>
                  <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left', overflow: 'hidden', minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 650, color: TK.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
  ...
  const dockedStyle = {
    ...
    borderRadius: 0, background: TK.surface,
  };
  const floatingStyle = {
    position: 'fixed', top: 10, bottom: 10, [isRTL ? 'right' : 'left']: 10,
    borderRadius: RADIUS.xl, border: `1px solid ${TK.border}`,
    background: 'rgba(255,255,255,.98)', backdropFilter: 'blur(20px) saturate(1.2)', WebkitBackdropFilter: 'blur(20px) saturate(1.2)',
    boxShadow: SHADOW.lg,
  };
      ...
      {mobileOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 60 }} role="dialog" aria-modal="true" aria-label={isRTL ? 'التنقل' : 'Navigation'}>
          ...
          <div style={{
            ...
            width: 'min(292px, 88vw)', background: TK.surface,
            borderRight: isRTL ? 'none' : `1px solid ${TK.border}`,
            borderLeft: isRTL ? `1px solid ${TK.border}` : 'none',
